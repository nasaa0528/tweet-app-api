...
const addPostHandler = async (req, res, next) => {
  try {
    ...
    if (post.length === 0)
      throw { "status": 400, "statusTxt": "Post field cannot be empty", "data": "post field cannot be empty" };
    const postRef = await db.collection('posts').add({
      user: user,
      post: post,
      created: config.timestamp
    });
    res.json({ "msg": "Post is added successfully with id: " + postRef.id });
  } catch (err) {
...
const postsHandler = async (req, res, next) => {
  try {
    ...
    var postsSnap; 
    if (homeFlag){
      postsSnap = await db.collection('posts').orderBy('created', 'desc').get();
    }
    else {
      postsSnap = await db.collection('posts').where('user.id', '==', userId).orderBy('created', 'desc').get();
    }
    var meta = { 'total': postsSnap.size, 'requested_page': page };
    postsSnap = postsSnap.docs.slice(startAt, endAt);
    var posts = [];
